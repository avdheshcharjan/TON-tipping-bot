...
async def welcome_handler(message: types.Message):
    ...
    await message.answer(
        "Feel like a volunteer did a great job? Show your appreciation with a tip in TON! 💸\n"
        "[Here is](https://www.coingecko.com/learn/how-to-set-up-ton-wallet-crypto) a guide on how to tip!",
...
async def balance_handler(message: types.Message):
    ...
    await message.answer(
        f"Your balance: *{user_balance:.2f} TON*", parse_mode=ParseMode.MARKDOWN
    )
...
async def deposit_handler(message: types.Message):
    ...
    button = InlineKeyboardButton(
        "Deposit", url=f"ton://transfer/{config.DEPOSIT_ADDRESS}&text={uid}"
    )
    keyboard.add(button)
